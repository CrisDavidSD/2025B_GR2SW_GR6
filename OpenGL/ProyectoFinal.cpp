...
glm::vec3 item1Pos = glm::vec3(5.0f, 0.0f, -5.0f);
glm::vec3 item2Pos = glm::vec3(-0.571307f, -2.0f, 4.21f);
bool haveItem1 = false;
bool haveItem2 = false;

// ---------------------------------------------------------
// ESTRUCTURA DE ZONAS CAMINABLES
// ---------------------------------------------------------
struct WalkableZone {
    float minX, maxX;
    float minZ, maxZ;
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (!haveItem1 && glm::distance(camera.Position, item1Pos) < 1.5f) {
            haveItem1 = true;
            itemsCollected++;
            std::cout << "Item 1 recogido!" << std::endl;
        ...
        if (!haveItem2 && glm::distance(camera.Position, item2Pos) < 1.5f) {
            haveItem2 = true;
            itemsCollected++;
            std::cout << "Item 2 recogido! SCREAMER!" << std::endl;
            if (flashlightSound) Mix_PlayChannel(-1, flashlightSound, 0);
        ...
        sceneShader.setFloat("spotLight.linear", (itemsCollected > 0) ? 0.14f : 0.09f);
        sceneShader.setFloat("spotLight.quadratic", (itemsCollected > 0) ? 0.07f : 0.032f);
        ...
        if (!haveItem1) {
            model = glm::mat4(1.0f);
            model = glm::translate(model, item1Pos);
        ...
        if (!haveItem2) {
            model = glm::mat4(1.0f);
            model = glm::translate(model, item2Pos);
        ...
        if (rainEnabled && rainShader) {
            ...
            for (auto& drop : rainDrops) {
                ...
                glm::vec3 toCam = camera.Position - drop.position;
                toCam.y = 0;
                if (glm::length(toCam) > 25.0f) {
                    drop.position = camera.Position + glm::vec3((rand() % 40) - 20, 15, (rand() % 40) - 20);
                }
                ...
                rainVertices.push_back(drop.position.x);
                rainVertices.push_back(drop.position.y - 0.3f);
                rainVertices.push_back(drop.position.z);
            }
...
void processInput(GLFWwindow* window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS) {
        std::cout << "Pos: " << nextPos.x << ", " << nextPos.y <<", " << nextPos.z << std::endl;
    }

    // lluvia
    static bool rPress = false;
    if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS && !rPress) {
        rainEnabled = !rainEnabled;
        rPress = true;
        if (rainEnabled) { if (rainSoundChannel == -1) rainSoundChannel = Mix_PlayChannel(-1, rainSound, -1); }
        else { Mix_HaltChannel(rainSoundChannel); rainSoundChannel = -1; }
    }
...
unsigned int loadCubemap(std::vector<std::string> faces) {
    ...
    for (unsigned int i = 0; i < faces.size(); i++) {
        ...
        else { std::cout << "Cubemap failed: " << faces[i] << std::endl; stbi_image_free(data); }
    }
